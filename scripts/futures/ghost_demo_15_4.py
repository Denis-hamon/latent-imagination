...
def main() -> int:
    ...
    for s in SCENARIOS:
        ...
        sdir = SESS / f"scenario-s{s}"
        sdir.mkdir(exist_ok=True)
        import shutil
        shutil.copy(SESS / "session-embeds.npz", sdir / "session-embeds.npz")
        m2 = dict(mani)
        m2["session_id"] = f"ghost-session-v9/scenario-s{s}"
        m2["out_dir"] = str(sdir)
        m2["budget_n"] = N_BOOT
    ...
    g2_ok = all(v["regime"] == "fallback-prior" and v["n_bootstrap"] == N_BOOT and v["grounded_by"] == "tests-run"
                for v in verdicts)
    verdict = {
        "demo": "ghost-demo-15-4", "prereg_anchor": "1babd393",
    ...
    (SESS / "demo-verdict.json").write_text(json.dumps(verdict, indent=1, ensure_ascii=False) + "\n")
    print("\n=== VERDICT DÉMO ===")
