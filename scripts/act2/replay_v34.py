...
OUT = NH / "replay-v34"
ROWS = NH / "replay-rows-v34.jsonl"
COUNTER = NH / "call-counter-v34.jsonl"
...
def select_tickets() -> list[dict]:
    ...
    if v32.is_file():
        ...
        SELECTION.write_text(json.dumps({"window": "v34", "anchor": "1d18e8742ffa26a9",
    ...
    sel = cands[:24]
    OUT.mkdir(parents=True, exist_ok=True)
    SELECTION.write_text(json.dumps({"window": "v34", "anchor": "e223b620dbc52228",
...
def measure_cumulative(t: dict, wt: str, san: str, check_files: list | None = None) -> dict:
    ...
    tmp = pathlib.Path(f"/tmp/v34-{abs(hash(san)) % 10**8}.diff")
    tmp.write_text(san if san.endswith("\n") else san + "\n")
    rf = f"/tmp/v34-{abs(hash(san)) % 10**8}.diff"
...
def replay_ticket(t: dict, model_key: str, model: str, fh) -> None:
    ...
    wt = f"{prof['wt_root']}/v34-{abs(hash((t['issue'], model))) % 10**8}"
    ...
    if t.get("test_patch"):
        ...
        if not ap["applied"]:
            fh.write(json.dumps({"id": f"v34-setup-{t['issue']}", "issue": t["issue"], "model": model,
    ...
    try:
        for turn in range(1, MAX_TURNS + 1):
            ...
            cid = f"v34-{t['repo']}-{model_key}-{slug[-40:]}-t{turn}"
            try:
                rth.MODEL["m"] = model
...
def main() -> int:
    ...
    print(f"REPLAY v34 : {len(sel)} tickets × modèles {[k for k, _ in models]} "
    ...
    print("REPLAY V34 DONE", flush=True)
    return 0
