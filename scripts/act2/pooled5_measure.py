#!/usr/bin/env python3
"""NIGHT-HARVEST-v1 — mesure pooled5 (jina) : poison ext-LOAO + AUC/IC
bootstrap + breakdown par famille-auteur. Même protocole que pooled4/sweep.
Run: uv run python scripts/act2/pooled5_measure.py
"""
...
def main() -> int:
    rows = json.loads((P5 / "pooled5-rows.json").read_text())
    d = np.load(P5 / "pooled5-embed.npz")
    ...
    for r in rows:
        if r.get("campaign") == "night-harvest-v1":
            ...
            st[r["y"]] += 1
    report = {"population": "pooled5", "generated_at": datetime.now(UTC).isoformat().replace("+00:00", "Z"),
    ...
    (ROOT / "governance" / "act2" / "arm-artifacts" /
     f"pooled5-measure-{datetime.now(UTC).strftime('%Y-%m-%d-%H%M')}.json").write_text(
