#!/usr/bin/env python3
"""Fenêtre v25 — replay voie propre : Pro+GLM sur tickets vérifiés kimi/epv nouveaux.
...
OUT = NH / "replay-v25"
ROWS = NH / "replay-rows-v25.jsonl"
COUNTER = NH / "call-counter-v25.jsonl"
SELECTION = OUT / "replay-selection-v25.json"
CAP_TOTAL = 320
CAP_MODEL = 160
MAX_TURNS = 4
FEEDBACK_CHARS = 800
...
REPOS_V22 = ("kimi", "epv", "tanquery", "qwen")
...
def select_tickets() -> list[dict]:
    # sélection figée : tickets vérifiés (RED-GREEN réels) des repos à petits
    # fichiers ; F2P >= 2 ; total src au parent <= 1200 lignes => prompt COMPLET
    cands = []
    for repo in REPOS_V22:
        f = NH / repo / "verified.json"
        if not f.is_file():
            continue
        for t in json.loads(f.read_text()):
            t.setdefault("repo", repo)
            if len(t.get("f2p", [])) < 2 or not t.get("fix_commit"):
                continue
            if repo in rth.REPOS:
                n = _src_total_lines(t)
                if n > MAX_TOTAL_SRC_LINES:
                    continue
                t["src_total_lines"] = n
            cands.append(t)
    deja_v22 = set()
    sel22 = OUT.parent / "replay-v22" / "replay-selection-v22.json"
    if sel22.is_file():
        deja_v22 = {t["issue"] for t in json.loads(sel22.read_text())["tickets"]}
    cands = [t for t in cands if t["issue"] not in deja_v22]
    cands.sort(key=lambda t: (-len(t["f2p"]), t["issue"]))
    sel = cands[:40]
    OUT.mkdir(parents=True, exist_ok=True)
    SELECTION.write_text(json.dumps({"window": "v25", "anchor": "v25-collecte-propre",
                                     "selected_at": datetime.now(UTC).isoformat().replace("+00:00", "Z"),
                                     "critere": "F2P>=2 ; src_total<=1200 ; verified RED-GREEN",
...
def replay_ticket(t: dict, model_key: str, model: str, fh) -> None:
    ...
    try:
        for turn in range(1, MAX_TURNS + 1):
            ...
            cid = f"v25-{t['repo']}-{model_key}-{slug[-40:]}-t{turn}"
            try:
                rth.MODEL["m"] = model
...
def main() -> int:
    ...
    print(f"REPLAY v25 : {len(sel)} tickets × modèles {[k for k, _ in models]} "
    ...
    print("REPLAY V25 DONE", flush=True)
    return 0
