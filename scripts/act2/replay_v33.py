...
OUT = NH / "replay-v33"
ROWS = NH / "replay-rows-v33.jsonl"
COUNTER = NH / "call-counter-v33.jsonl"
...
def select_tickets() -> list[dict]:
    v32 = NH / "replay-v32" / "replay-selection-v32.json"
    if v32.is_file():
        sel = json.loads(v32.read_text())["tickets"]
        OUT.mkdir(parents=True, exist_ok=True)
        SELECTION.write_text(json.dumps({"window": "v33", "anchor": "3096c0f90178f8dc",
                                         "heritage": "replay-selection-v32.json IDENTIQUE",
    ...
    sel = cands[:24]
    OUT.mkdir(parents=True, exist_ok=True)
    SELECTION.write_text(json.dumps({"window": "v33", "anchor": "e223b620dbc52228",
...
def measure_cumulative(t: dict, wt: str, san: str, check_files: list | None = None) -> dict:
    ...
    tmp = pathlib.Path(f"/tmp/v33-{abs(hash(san)) % 10**8}.diff")
    tmp.write_text(san if san.endswith("\n") else san + "\n")
    rf = f"/tmp/v33-{abs(hash(san)) % 10**8}.diff"
...
def replay_ticket(t: dict, model_key: str, model: str, fh) -> None:
    ...
    wt = f"{prof['wt_root']}/v33-{abs(hash((t['issue'], model))) % 10**8}"
    ...
    if t.get("test_patch"):
        ...
        if not ap["applied"]:
            fh.write(json.dumps({"id": f"v33-setup-{t['issue']}", "issue": t["issue"], "model": model,
    ...
    try:
        for turn in range(1, MAX_TURNS + 1):
            ...
            cid = f"v33-{t['repo']}-{model_key}-{slug[-40:]}-t{turn}"
            try:
                rth.MODEL["m"] = model
...
def main() -> int:
    want = os.environ.get("GEN_MODELS", "pro,glm,gemma").split(",")
    ...
    print(f"REPLAY v33 : {len(sel)} tickets × modèles {[k for k, _ in models]} "
    ...
    print("REPLAY V33 DONE", flush=True)
    return 0
