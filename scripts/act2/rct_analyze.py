...
Mesure primaire : ΔF2P-pass par tâche, McNemar exact sur discordances,
publié quel que soit le signe (prereg). Sortie : rct-v1/analysis.json + table.
"""
...
def load_outcome(d: Path) -> bool | None:
    f = d / "run-result.json"
    if not f.is_file():
        return None
...
def main() -> int:
    ...
    A = [load_outcome(PILOT / "results" / f"{k}-off") for k in keys]
    B0 = [load_outcome(RCT / "results" / f"{k}-b0") for k in keys]
    B1 = [load_outcome(RCT / "results" / f"{k}-b1") for k in keys]

    out = {"panel": "frozen32", "arms_present": {
               "A": sum(x is not None for x in A),
               "B0": sum(x is not None for x in B0),
               "B1": sum(x is not None for x in B1)},
           "B1_vs_A": mcnemar(A, B1), "B0_vs_A": mcnemar(A, B0), "B1_vs_B0": mcnemar(B0, B1)}
    (RCT / "analysis.json").write_text(json.dumps(out, indent=1))

    print(f"arms présentes : {out['arms_present']}")
    for cmp_, m in (("B1 vs A ", out["B1_vs_A"]), ("B0 vs A ", out["B0_vs_A"]),
                    ("B1 vs B0", out["B1_vs_B0"])):
        print(f"{cmp_} | n={m['n_paired']:2d} | F2P {m['a_rate']:.3f} vs {m['b_rate']:.3f} | "
