#!/usr/bin/env python3
"""Window coverage-ts-v4 (préparation — pilote OmniRoute) — DIFFICULTY-PROBE sur worldmonitor
...
MUTANTS = [
    {"task_id": "omniroute__lite.backward_polarity_inverted", "tier": "ts4-easy",
     ...
     "problem": "La compression des résultats d'outils coupe des mots en deux : "
                "le texte tronqué se termine au milieu d'un mot (ex. 'authentic' "
                "au lieu de reculer jusqu'à l'espace précédent). Le contenu "
                "compressé doit toujours couper sur une frontière de mot."},
    {"task_id": "omniroute__lite.max_length_raised", "tier": "ts4-easy",
     ...
     "problem": "Les résultats d'outils de quelques milliers de caractères ne sont "
                "plus du tout tronqués et passent intégralement dans le contexte, "
                "alors qu'ils devraient être compressés à ~2000 caractères avec le "
    ...
    {"task_id": "omniroute__lite.triple_coordinated", "tier": "ts4-hard",
     ...
     "replacements": [
        ...
        ("const MAX_TOOL_LENGTH = 2000;",
         "const MAX_TOOL_LENGTH = 20000;")],
     "problem": "La compression lite des résultats d'outils ne fonctionne plus du "
                "tout : les messages d'outils longs ne sont jamais tronqués, et "
                "quand une troncature a lieu elle coupe en plein mot au lieu de "
                "reculer sur une frontière."},
]
...
def author_stage() -> int:
    ...
    os.environ["PILOT_CAMPAIGN_DIR"] = "coverage-ts-4"
    pr.os.environ["PILOT_CAMPAIGN_DIR"] = "coverage-ts-4"
