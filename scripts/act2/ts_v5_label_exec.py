#!/usr/bin/env python3
"""Story coverage-ts-v5 — labellisation stricte DISTANTE (node --test sur worldmonitor).
...
Q = ROOT / "data" / "landing" / "act2-pilot" / "coverage-ts-5"
...
def node_test_remote(spec: str) -> dict:
    """node --test natif, reporter TAP ; noms au niveau `it` (lignes ok/not ok,
    y compris sous-tests indentés)."""
    r = sh_remote(f"cd {REMOTE} && node --import tsx/esm --test --test-reporter=tap {spec} 2>&1",
                  timeout=900)
    raw = r.stdout
    per_test = []
    for line in raw.splitlines():
        ...
        if l.startswith("not ok "):
            per_test.append({"name": l[7:].split(" # ")[0].strip(), "status": "failed"})
        elif l.startswith("ok "):
            per_test.append({"name": l[3:].split(" # ")[0].strip(), "status": "passed"})
    return {"rc": r.returncode, "per_test": per_test, "tail": raw[-600:]}


def _check_clean() -> str:
    return sh_remote(f"cd {REMOTE} && git -C ~/kimsufi-source status --porcelain | head -1").stdout.strip()


def run_slot(slot_dir: Path, task: dict) -> dict:
    rec = json.loads((slot_dir / "rec.json").read_text())
    iid = rec["task"]
    out = {"task": iid, "slot": slot_dir.name, "campaign": "coverage-ts-2",
           "window": "coverage-ts-v2", "author": rec.get("author"),
    ...
    try:
        ...
        vbug = node_test_remote(task["spec"])
        ...
        sh_remote(f"cd {REMOTE} && git checkout -- {task['target']} && rm -f /tmp/genfam-cand.diff",
                  timeout=120)
        if _check_clean():
...
def main() -> int:
    ...
    print(f"\n== TSv5-L : {done} déjà mesurés, {run} exécutés, {errors} erreurs ==")
    return 0
