...
OUT = NH / "replay-v38"
ROWS = NH / "replay-rows-v38.jsonl"
COUNTER = NH / "call-counter-v38.jsonl"
SELECTION = OUT / "replay-selection-v38.json"
CAP_TOTAL = 24
CAP_MODEL = 24
MAX_TURNS = 4
FEEDBACK_CHARS = 1600
...
def select_tickets() -> list[dict]:
    """v38 : les 6 irréductibles v36/v37 (2 vue + 4 dayjs), explicites."""
    irr = ["vuejs__core-11854", "vuejs__core-9572",
           "iamkun__dayjs-1047", "iamkun__dayjs-1502",
           "iamkun__dayjs-1964", "iamkun__dayjs-873"]
    sel = []
    for repo, vf in (("vuejs__core", "mswb/vuejs__core/verified-mswb.json"),
                     ("iamkun__dayjs", "mswb/iamkun__dayjs/verified-mswb.json")):
        for t in json.loads((NH.parent / vf).read_text()):
            if t["issue"] in irr:
                t.setdefault("repo", repo)
                sel.append(t)
    OUT.mkdir(parents=True, exist_ok=True)
    SELECTION.write_text(json.dumps({"window": "v38", "anchor": "bf509d269955badc",
                                     "critere": "6 irréductibles v36/v37 explicites",
...
def replay_ticket(t: dict, model_key: str, model: str, fh) -> None:
    ...
    try:
        for turn in range(1, MAX_TURNS + 1):
            ...
            cid = f"v38-{t['repo']}-{model_key}-{slug[-40:]}-t{turn}"
            try:
                rth.MODEL["m"] = model
