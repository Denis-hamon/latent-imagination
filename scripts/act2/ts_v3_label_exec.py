#!/usr/bin/env python3
"""Story coverage-ts-v3 — labellisation stricte DISTANTE (node --test sur worldmonitor).
...
def node_test_remote(spec: str) -> dict:
    ...
    r = sh_remote(f"cd {REMOTE} && node --test --test-reporter=tap {spec} 2>&1",
                  timeout=900)
    raw = r.stdout
    per_test = []
...
def main() -> int:
    ...
    print(f"\n== TSv3-L : {done} déjà mesurés, {run} exécutés, {errors} erreurs ==")
    return 0
