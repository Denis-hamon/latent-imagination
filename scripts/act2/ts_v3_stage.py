#!/usr/bin/env python3
"""Window coverage-ts-v3 — staging : quota-tasks validés → répertoire campagne
coverage-ts-3 (staging-extract.json + buggy sources) pour le harness budget.
...
SRC = ROOT / "data" / "landing" / "act2-pilot" / "ts-v3"
OUT = ROOT / "data" / "landing" / "act2-pilot" / "coverage-ts-3"
...
def main() -> int:
    ...
    blob = json.dumps({"window": "coverage-ts-v3",
