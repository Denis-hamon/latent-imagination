...
AP = argparse.ArgumentParser(); AP.add_argument("--dir", default="coverage-ts-8-flash")
...
def run_slot(slot_dir: Path, task: dict) -> dict:
    ...
    out = {"task": iid, "slot": slot_dir.name, "campaign": ARGS.dir,
           "window": "coverage-ts-v8", "author": rec.get("author"),
...
def main() -> int:
    ...
    print(f"\n== TSv8-L : {done} déjà mesurés, {run} exécutés, {errors} erreurs ==")
    return 0
