#!/usr/bin/env python3
"""Story coverage-ts-v2 — labellisation stricte DISTANTE (vitest sur Kimsufi).
...
Q = ROOT / "data" / "landing" / "act2-pilot" / "coverage-ts-2"
...
def vitest_remote(spec: str) -> dict:
    ...
    try:
        ...
        for tr in d.get("testResults", []):
            for a in tr.get("assertionResults", []):
                per_test.append({"name": a.get("title", ""), "status": a.get("status")})
...
def run_slot(slot_dir: Path, task: dict) -> dict:
    ...
    try:
        ...
        if not applied:
            return {**out, "apply_err": (ap.stdout + ap.stderr)[-300:]}
        # 3) vitest complet du spec
        v = vitest_remote(task["spec"])
...
def main() -> int:
    ...
    print(f"\n== TSv2-L : {done} déjà mesurés, {run} exécutés, {errors} erreurs ==")
    return 0
