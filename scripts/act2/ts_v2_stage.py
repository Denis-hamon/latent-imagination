#!/usr/bin/env python3
"""Window coverage-ts-v2 — staging : quota-tasks validés → répertoire campagne
coverage-ts-2 (staging-extract.json + buggy sources) pour le harness budget.
...
SRC = ROOT / "data" / "landing" / "act2-pilot" / "ts-v2"
OUT = ROOT / "data" / "landing" / "act2-pilot" / "coverage-ts-2"
...
def main() -> int:
    ...
    blob = json.dumps({"window": "coverage-ts-v2",
