...
MUTANTS = [
    {"task_id": "omniroute__lite.double_polarity_maxlen",
     ...
     "problem": "La compression des résultats d'outils a deux symptômes : les "
                "contenus de quelques milliers de caractères ne sont plus du tout "
                "tronqués, et quand une troncature a tout de même lieu elle coupe "
                "en plein mot au lieu de reculer sur une frontière."},
]
...
def verify_stage() -> int:
    ...
    (Q / "probe-manifest.json").write_text(json.dumps(
        {"window": "coverage-ts-v5", "stage": "verify", "mutants": results,
...
def author_stage() -> int:
    ...
    os.environ["PILOT_CAMPAIGN_DIR"] = "coverage-ts-5"
    pr.os.environ["PILOT_CAMPAIGN_DIR"] = "coverage-ts-5"
    ...
    for m in valid:
        ...
        row = {"ts": datetime.now(UTC).isoformat(), "window": "coverage-ts-v5",
