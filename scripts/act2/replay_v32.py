...
OUT = NH / "replay-v32"
ROWS = NH / "replay-rows-v32.jsonl"
COUNTER = NH / "call-counter-v32.jsonl"
...
MODELS = {
    ...
    "nemotron": "NVIDIA-Nemotron-3-Super-120B-A12B-MLX",
}
...
def select_tickets() -> list[dict]:
    ...
    SELECTION.write_text(json.dumps({"window": "v32", "anchor": "e223b620dbc52228",
...
def measure_cumulative(t: dict, wt: str, san: str, check_files: list | None = None) -> dict:
    ...
    tmp = pathlib.Path(f"/tmp/v32-{abs(hash(san)) % 10**8}.diff")
    tmp.write_text(san if san.endswith("\n") else san + "\n")
    rf = f"/tmp/v32-{abs(hash(san)) % 10**8}.diff"
...
def replay_ticket(t: dict, model_key: str, model: str, fh) -> None:
    ...
    wt = f"{prof['wt_root']}/v32-{abs(hash((t['issue'], model))) % 10**8}"
    ...
    if t.get("test_patch"):
        ...
        if not ap["applied"]:
            fh.write(json.dumps({"id": f"v32-setup-{t['issue']}", "issue": t["issue"], "model": model,
    ...
    try:
        for turn in range(1, MAX_TURNS + 1):
            ...
            cid = f"v32-{t['repo']}-{model_key}-{slug[-40:]}-t{turn}"
            try:
                rth.MODEL["m"] = model
...
def main() -> int:
    ...
    print(f"REPLAY v32 : {len(sel)} tickets × modèles {[k for k, _ in models]} "
    ...
    print("REPLAY V32 DONE", flush=True)
    return 0
