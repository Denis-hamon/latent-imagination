...
CAMPAIGN = "coverage-ts-6"  # campagne/ledger v6 — bug DW-31 de v5 évité

PROBE_TASKS = [
    "omniroute__trc.double_json_guard_errline",
    "omniroute__hb.double_preserve_proportional",
]
DRAWS = 2
...
def author_stage() -> int:
    ...
    for tid in PROBE_TASKS:
        ...
        for d in range(1, DRAWS + 1):
            ...
            row = {"ts": datetime.now(UTC).isoformat(), "window": "coverage-ts-v6",
    ...
    (Q / "probe-verdict-v6.json").write_text(json.dumps(
        {"window": "coverage-ts-v6", "campaign": CAMPAIGN,
