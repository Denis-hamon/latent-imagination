#!/usr/bin/env python3
"""Window coverage-ts-v10 — SONDE PRÉ-GEL A/B : lite.triple_coordinated +
...
ARMS = {"flash": ("coverage-ts-10", "DeepSeek-V4-Flash")}
PROBE_TASKS = ["zod__checks.triple_minmax_bag",
               "date_fns__bizdays.triple_weeks_ternary_start"]
...
def main() -> int:
    ...
    for arm, (cdir, model) in ARMS.items():
        ...
        for tid in PROBE_TASKS:
            ...
            row = {"ts": datetime.now(UTC).isoformat(), "window": "coverage-ts-v10",
    ...
    out = ROOT / "data/landing/act2-pilot" / "ts-v10" / "probe-v10-verdict.json"
    out.write_text(json.dumps({"window": "coverage-ts-v10",
