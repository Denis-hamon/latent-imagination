#!/usr/bin/env python3
"""Story coverage-ts-v6 — labellisation stricte DISTANTE (node --test sur worldmonitor).
...
Q = ROOT / "data" / "landing" / "act2-pilot" / "coverage-ts-6"
...
def run_slot(slot_dir: Path, task: dict) -> dict:
    ...
    iid = rec["task"]
    out = {"task": iid, "slot": slot_dir.name, "campaign": "coverage-ts-6",
...
def main() -> int:
    ...
    print(f"\n== TSv6-L : {done} déjà mesurés, {run} exécutés, {errors} erreurs ==")
    return 0
