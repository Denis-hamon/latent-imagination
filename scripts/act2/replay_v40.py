...
OUT = NH / "replay-v40"
ROWS = NH / "replay-rows-v40.jsonl"
COUNTER = NH / "call-counter-v40.jsonl"
SELECTION = OUT / "replay-selection-v40.json"
CAP_TOTAL = 400
CAP_MODEL = 200
MAX_TURNS = 4
FEEDBACK_CHARS = 1600
...
def select_tickets() -> list[dict]:
    """v40 : tickets minés vérifiés (kimi/qwen/epv), jamais joués en recette
    65k, F2P>=2, src<=1200 lignes. Figée avant appels."""
    deja = set()
    for vdir in ("replay-v22", "replay-v25"):
        sf = NH / vdir / f"replay-selection-{vdir[-2:]}.json"
        if sf.is_file():
            deja |= {t["issue"] for t in json.loads(sf.read_text())["tickets"]}
    cands = []
    for repo in ("kimi", "qwen", "epv"):
        f = NH / repo / "verified.json"
        if not f.is_file():
            continue
        for t in json.loads(f.read_text()):
            t.setdefault("repo", repo)
            if t["issue"] in deja or len(t.get("f2p", [])) < 2 or not t.get("fix_commit"):
                continue
            cands.append(t)
    for t in cands:
        n = _src_total_lines(t)
        t["src_total_lines"] = n if n < 10**9 else 0
    cands = [t for t in cands if 0 < t["src_total_lines"] <= MAX_TOTAL_SRC_LINES]
    cands.sort(key=lambda t: (-len(t["f2p"]), t["issue"]))
    sel = cands[:60]
    OUT.mkdir(parents=True, exist_ok=True)
    SELECTION.write_text(json.dumps({"window": "v40", "anchor": "b479285c70090bf2",
                                     "critere": "minés vérifiés non joués 65k ; F2P>=2 ; src<=1200 ; top 60 par F2P",
...
DIFF_ONLY_PREFIX = ("Keep your reasoning SHORT (under 5000 tokens). Then respond "
                    "with ONLY the content of a single ```diff block. "
                    "No explanations, no reasoning text outside the block.\n\n")


def extract_tolerant(text: str) -> str:
    """v38 infra : si la fence fermante manque (réponse tronquée), récupérer le
    diff depuis le dernier bloc ```diff jusqu'à EOF — hunks utilisables même
    coupés. Retourne '' si aucun diff plausible."""
    if "```diff" not in text:
        return ""
    chunk = text.rsplit("```diff", 1)[-1]
    chunk = chunk.split("```", 1)[0]  # retire une éventuelle fence fermante
    chunk = chunk.strip()
    return chunk if chunk.startswith(("diff --git", "---", "++@", "@@")) or "diff --git" in chunk[:200] else ""
...
def replay_ticket(t: dict, model_key: str, model: str, fh) -> None:
    ...
    wt = f"{prof['wt_root']}/v40-{abs(hash((t['issue'], model))) % 10**8}"
    ...
    if t.get("test_patch"):
        ...
        if not ap["applied"]:
            fh.write(json.dumps({"id": f"v40-setup-{t['issue']}", "issue": t["issue"], "model": model,
            ...
            return
    elif t.get("fix_commit"):
        # tickets minés : les fichiers de tests viennent du commit de fix
    ...
    try:
        for turn in range(1, MAX_TURNS + 1):
            ...
            cid = f"v40-{t['repo']}-{model_key}-{slug[-40:]}-t{turn}"
            try:
                rth.MODEL["m"] = model
                ...
                fh.flush()
                break
            (tdir / f"t{turn}.raw").write_text(g["text"])
            san = rth.PRX.extract_diff_sanitized(g["text"])
            if not san:
                san = extract_tolerant(g["text"])
                tolerante = bool(san)
            else:
                tolerante = False
