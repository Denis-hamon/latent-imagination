...
echo "==> fresh venv, install ONLY wheel-rolled deps the bundle can verify."
python3 -m venv "$WORK/venv" --symlinks
"$WORK/venv/bin/python" -m ensurepip --upgrade 2>/dev/null || true
# duckdb is a reproduction variable: pin the same version as the workspace.
"$WORK/venv/bin/pip" install --quiet "duckdb==1.5.5"

echo "==> follow STRANGER.md: recompute (isolated interpreter, no ambient imports)"
"$WORK/venv/bin/python" -I "$BUNDLE/pipeline/run.py" \
