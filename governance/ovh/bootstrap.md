...
| 2026-08-06 | **Live OTS anchor obtained** (same chain, from the builder host — node's egress to calendars failed at ceremony) | `ots stamp` + `ots info` | proof `PendingAttestation(finney.calendar.eternitywall.com)` committed at `data/release-store/proofs/3ff03b8a7f393c57.ots`; row appended to the node ledger; upgrade path stays per `governance/anchor-fallback.md`. Root cause of the node's failure found + fixed: adapter invoked `python -m ots` — no such module in pinned 0.7.2 (console script `ots`; FILE is positional, no `-i`). Live path now exercised by an opt-in integration test (`LI_OTS_LIVE=1`). |
